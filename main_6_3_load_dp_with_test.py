...
class build_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.label == 'train': # train
            ...
            mask = zoom(mask, (h_resize/h, w_resize/w, 1, 1))   #use resizing instead of padding
            ...
            mask = zoom(mask, (h_resize/h, w_resize/ w, 1, 1))  # 144/d))   #use resizing instead of padding
            mask = np.pad(mask, ((0, 0), (0, 0), (0, 0), (padding_depth, padding_depth)))
            if np.max(mask)>0:
                mask/=np.max(mask)
            mask[mask<0.5]=0
