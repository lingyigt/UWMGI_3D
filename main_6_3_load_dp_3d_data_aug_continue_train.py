...
def masks2rles(msks, ids, heights, widths, slice_num):
    ...
    for idx in range(msks.shape[0]):
        ...
        id_slice = ids[idx] + '_slice_' + str(slice_num).zfill(4)
        pred_ids.extend([ids[idx]]*len(rle))
        pred_classes.extend(['large_bowel', 'small_bowel', 'stomach'])
    return pred_strings, pred_ids, pred_classes


def masks2rles_singleslice(msk, id, height, width, slice_num):
    pred_strings = []; pred_ids = []; pred_classes = [];
    msk = cv2.resize(msk,
                    dsize=(width, height),
                    interpolation=cv2.INTER_NEAREST) # back to original shape
    ...
    for midx in [0, 1, 2]:
        rle[midx] = mask2rle(msk[..., midx])
    pred_strings.extend(rle)
    id_slice = id + '_slice_' + str(slice_num+1).zfill(4)
    pred_ids.extend([id_slice]*len(rle))
    pred_classes.extend(['large_bowel', 'small_bowel', 'stomach'])
    return pred_strings, pred_ids, pred_classes
###############################################################
...
def build_transforms(CFG):
    data_transforms = {
        "train": Compose([GaussianBlur3D(sigma=[.1, 2.], execution_probability=0.5)]),
...
class build_dataset(Dataset):
    def __init__(self, df, label='train', transforms=None, cfg=None):
        ...
        if 'img_3d_path' in df.columns:
            self.img_3d_paths = df['img_3d_path'].tolist()
        else:
            self.img_3d_paths = None

        if 'mask_3d_path' in df.columns:
            self.mask_3d_paths = df['mask_3d_path'].tolist()
        else:
            self.mask_3d_paths = None
    ...
    def __getitem__(self, index):
        ...
        if self.label == 'train': # train #TODO:Add transform here, but later
            #load 3d img
            img_3d_path = self.img_3d_paths[index]
            img = np.load(img_3d_path)
            #transform
            img = self.transforms(img)
            #load 3d mask
            mask_3d_path = self.mask_3d_paths[index]
            mask = np.load(mask_3d_path)
            ### load distance map
        ...
        elif self.label == 'val': # validation
            #load 3d img
            img_3d_path = self.img_3d_paths[index]
            img = np.load(img_3d_path)
            #load 3d mask
            ...
            img = zoom(img, (h_resize / h, w_resize / w, 1))  # 144/d))  #use resizing instead of padding
            ...
            return torch.tensor(img), id, h, w, d, padding_depth
...
def build_model(CFG, test_flag=False):
    ...
    model.load_state_dict(torch.load(CFG.ckpt_path_toload[0]))
    model.to(CFG.device)
    return model
...
def HD_coeff(y_true,y_pred,CFG):  # Note: y_pred should be thresholded
    ...
    for i in range(y_true.shape[0]):
        for j in range(CFG.num_classes):
            ...
            dp_y_pred = get_dp(y_pred_cpu[i, j, :, :, :], CFG)
...
def get_dp(input, CFG):  # input dimension [d, h, w]: get 3d distance map for one class input needs to be on cpu to perform ndimage.distance_transform_edt
    h_resize, w_resize, d_resize = CFG.img_size
    dp_inside = ndimage.distance_transform_edt(input, sampling=[1/d_resize, 1/h_resize, 1/w_resize]) #sampling (normalization)
...
def test_one_epoch(ckpt_paths, test_loader, CFG):
    ...
    for _, (images, ids, h, w, d, padding_depth) in pbar:

        images  = images.to(CFG.device, dtype=torch.float) #  [b, c, d, h, w]
        size = images.size()
        masks = torch.zeros((size[0], 3, size[2], size[3], size[4]), device=CFG.device, dtype=torch.float32) # [b, c, d, h, w]
        ############################################
        ...
        masks = (masks.permute((0, 2, 3, 4, 1))>CFG.thr).to(torch.uint8).cpu().detach().numpy() # [b, d, h, w, c]
        for sample_num in range(images.shape[0]):
            for slice_num in range(d[sample_num].item()):
                masks_slice = masks[sample_num, slice_num+padding_depth[sample_num].item(), :, :, :]
                result = masks2rles_singleslice(masks_slice, ids[sample_num], h[sample_num].item(), w[sample_num].item(), slice_num)
...
if __name__ == '__main__':
    ...
    class CFG:
        ...
        ckpt_name_load = "img244244180_bs1_fold1-fm16-hdloss-adjustlossratio-load-dp-3d-fullset"
        ckpt_name = 'img244244180_bs1_fold1-fm16-hdloss-adjustlossratio-data-aug-continue-train'
        # step2: data
        n_fold = 5
        ...
        load_ckpt_path = f"../input/{ckpt_fold}/{ckpt_name_load}"
        ckpt_path_toload = glob(f'{load_ckpt_path}/best*')

    set_seed(CFG.seed)
    ckpt_path_load = f"../input/{CFG.ckpt_fold}/{CFG.ckpt_name_load}"
    ...
    if train_val_flag:
        ...
        #generate img_3d_path
        img_3d_path = df[['3d_id','image_path']]
        img_3d_path['img_3d_splited'] = img_3d_path.image_path.map(lambda x:x[0])
        img_3d_path['prefix'] = img_3d_path.img_3d_splited.str.split('scan')
        img_3d_path['prefix'] = img_3d_path['prefix'].map(lambda x:x[0]+'img3d/img3d.npy')
        img_3d_path['img_3d_path']=img_3d_path.prefix.str.replace('//', '/')
        img_3d_path = img_3d_path.drop(columns = ['image_path','img_3d_splited','prefix'])
        df = df.merge(img_3d_path,on='3d_id')

        #generate mask_3d_path
        mask_3d_path = df[['3d_id','mask_path']]
        mask_3d_path['mask_3d_splited'] = mask_3d_path.mask_path.map(lambda x:x[0])
        mask_3d_path['prefix'] = mask_3d_path.mask_3d_splited.str.split('scan')
        mask_3d_path['prefix'] = mask_3d_path['prefix'].map(lambda x:x[0] + '3d/mask3d.npy')
        mask_3d_path['mask_3d_path'] = mask_3d_path.prefix.str.replace('//','/')
        mask_3d_path = mask_3d_path.drop(columns = ['mask_path', 'mask_3d_splited','prefix'])
        df = df.merge(mask_3d_path, on='3d_id')
        ...
        for fold in range(1):
            ...
            lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor = 0.5, patience = 5) #try this lr_scheduler as introduced in the HD paper
            ...
            for epoch in range(1, CFG.epoch+1):
                ...
                #lr_scheduler.step()
                losses_all = train_one_epoch(model, train_loader, optimizer, losses_dict, CFG, epoch) #for the REDUCELRONPLATEAU lr scheduler
                lr_scheduler.step(losses_all) #for the REDUCELRONPLATEAU lr scheduler
    ...
    if test_flag:
        ...
        df3 = test_df.id.to_frame()
        df3['3d_id_0'] = df3.id.str.split('_')
        df3['3d_id'] = df3['3d_id_0'].str[0] + '_' + df3['3d_id_0'].str[1]
        df3 = df3.drop(columns=['3d_id_0'])
        test_df = test_df.merge(df3, on=['id'])

        df4 = test_df.groupby(['3d_id'])['image_path'].agg(list).to_frame().reset_index()
        test_df = test_df.groupby(['3d_id']).head(1).reset_index(drop=True)
        test_df = test_df.drop(columns=['image_path'])
        test_df = test_df.merge(df4, on='3d_id')

        data_transforms = build_transforms(CFG)
        test_dataset = build_dataset(test_df, label=False, transforms=data_transforms['train'], cfg=CFG)  #set train but not really
