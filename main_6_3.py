...
class build_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.label == 'train': # train
            ...
            mask = np.transpose(mask, (2, 3, 0, 1))  # [h, w, c, d] => [c, d, h, w]
            return torch.tensor(img), torch.tensor(mask)
        else:  # test  #TODO:Add padding here, later
            ### augmentations
...
# Metric: 3D HD
def HD_coeff(y_true,y_pred):  # Note: y_pred should be thresholded
    total_hd_coeff = 0
    ...
    for i in range(y_true.shape[0]):
        for j in range(y_true.shape[1]):
            dp_y_true = get_dp(y_true_cpu[i,j,:,:,:])
            dp_y_pred = get_dp(y_pred_cpu[i,j,:,:,:])
            dp_y_true = dp_y_true/np.sqrt(y_true.shape[2]**2 + y_true.shape[3]**2 + y_true.shape[4]**2)
            dp_y_pred = dp_y_pred/np.sqrt(y_true.shape[2]**2 + y_true.shape[3]**2 + y_true.shape[4]**2)
            ...
            hd_total += hd
    hd_ave = hd_total/(y_true.shape[0]*y_true.shape[1])
...
def get_dp(input):  # input dimension [d, h, w]: get 3d distance map for one class input needs to be on cpu to perform ndimage.distance_transform_edt
    ...
    #dp_inside = cv2.distanceTransform(input, distanceType=cv2.DIST_L2, maskSize=5) # cannot deal with 3D
    input_inverse = input
    ...
    #dp_outside = cv2.distanceTransform(input_inverse, distanceType=cv2.DIST_L2, maskSize=5)  #cannot deal with 3D
...
def hd_loss(y_true, y_pred, CFG):
    ...
    for i in range(y_true.shape[0]):  # iterate through batch
        for j in range(y_true.shape[1]):   # iterate through class
            curr_class_dp = get_dp(y_true_cpu[i,j,:,:,:])
            #curr_class_dp_inside = torch.tensor(ndimage.distance_transform_edt(y_true_cpu[i, j, :, :, :]))    #[batchsize, 1, d, h, w]
            #y_true_cpu_invert_mask = y_true_cpu[i, j, :, :, :]
            #y_true_cpu_invert_mask[y_true_cpu_invert_mask==1]=-1
            ##y_true_cpu_invert_mask[y_true_cpu_invert_mask == 0] = 1
            #y_true_cpu_invert_mask[y_true_cpu_invert_mask == -1] = 0
            #curr_class_dp_outside = torch.tensor(ndimage.distance_transform_edt(y_true_cpu_invert_mask))
            #curr_class_dp = curr_class_dp_inside + curr_class_dp_outside

            curr_class_dp = torch.tensor(curr_class_dp).to(CFG.device, dtype=torch.float)
    ...
    ave_hd_loss = total_hd_loss/(y_true.shape[0]*y_true.shape[1])
    return ave_hd_loss
...
def train_one_epoch(model, train_loader, optimizer, losses_dict, CFG, epoch):
    ...
    for _, (images, masks) in pbar:
        ...
        with amp.autocast(enabled=True):
            ...
            dice_loss = losses_dict["3DdiceLoss"](y_preds, masks)   #argument order: (input, target) as written in the source code
            ...
            if epoch < 10:
                ...
                losses = 0.6 * ave_hd_loss + 0.4 * loss_ratio*dice_loss
            #losses = ave_hd_loss
...
def valid_one_epoch(model, valid_loader, CFG):
    ...
    for _, (images, masks) in pbar:
        ...
        #y_preds = y_preds > CFG.thr
        y_preds[y_preds > CFG.thr] = 1
        y_preds[y_preds < CFG.thr] = 0
        val_dice = dice_coef(masks, y_preds).cpu().detach().numpy()   #TODO:check the rightfulness of this function
        val_hd = HD_coeff(masks, y_preds)  # Transfer to cpu inside the function
...
if __name__ == '__main__':
    ...
    class CFG:
        ...
        ckpt_fold = "ckpt-liz-20220624-code3D"
        ckpt_name = "img224224_bs1_fold5_3d_gr8_gaussianblur_elastic_deformation-hdloss-adjustlossratio"
        ...
        # step3: model
        backbone = 'efficientnet-b2' # This is not needed for 3D
        num_classes = 3
        ...
        # Other parameters
        one_fold_train = True
        debug = True  # Running only 3000 data on my laptop
    set_seed(CFG.seed)
        ...
        for fold in range(1):
            ...
            for epoch in range(1, CFG.epoch+1):
                ...
                if not epoch % 5:
                    ...
                    best_score = max(best_val_score, val_score)
                    ...
                    print("epoch:%2d, time:%5.2fs, best:%6.4f\n" % (epoch, epoch_time, best_score), flush=True)
